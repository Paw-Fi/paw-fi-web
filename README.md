...
- **Banking Integrations (Plaid)**:
  - `plaid-create-link-token`: Generates a Plaid Link token for the authenticated user (and supports update mode when a connection ID is provided).
  - `plaid-exchange-public-token`: Exchanges the Link `public_token` for an access token, stores the encrypted token + Plaid `item_id`, and upserts all Plaid accounts into `bank_accounts`.
  - `plaid-sync-transactions`: Manual "Sync" endpoint triggered by the frontend button. Runs `/transactions/sync`, inserts/updates expenses, deletes removed transactions, and records a `bank_sync_audit` summary so the frontend only needs to refetch expenses afterward.
