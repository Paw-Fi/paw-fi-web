...
begin
  ...
  if array_length(deletable_account_ids, 1) is not null then
    ...
    where t.created_by_user_id = current_user_id
       or t.from_account_id = any(deletable_account_ids)
       or t.to_account_id = any(deletable_account_ids);

    delete from public.expenses e
    where e.account_id = any(deletable_account_ids);

    with removed_wallets as (
      delete from public.accounts a
      where a.id = any(deletable_account_ids)
      returning a.id
    )
...
comment on function public.reset_user_financial_data() is
  'Resets all user financial data, queues every live Plaid connection for provider removal, sanitizes local bank tokens, and deletes every user-owned wallet.';
