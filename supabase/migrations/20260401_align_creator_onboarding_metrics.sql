...
begin
  ...
  funnel_rows as (
    select * from (
      values
        (1, 'intro_seen', (select count(*) from session_stage_flags where saw_intro)),
        (2, 'preauth_started', (select count(*) from session_stage_flags where saw_preauth)),
        (3, 'account_preparing_seen', (select count(*) from session_stage_flags where saw_account_preparing)),
        (4, 'paywall_seen', (select count(*) from session_stage_flags where saw_paywall)),
        (5, 'subscribe_tapped', (select count(*) from sessions_enriched where effective_stage_rank >= 7)),
        (6, 'purchase_succeeded', (select count(*) from sessions_enriched where completed_purchase)),
        (7, 'postauth_seen', (select count(*) from session_stage_flags where saw_post_auth))
