...
begin
  ...
  select normalized_source as source, count(*) as count
  from normalized_responses
  group by normalized_source
  order by count(*) desc, normalized_source;
end;
$$;
...
create index if not exists onboarding_heard_about_responses_created_at_idx
  on public.onboarding_heard_about_responses (created_at);
