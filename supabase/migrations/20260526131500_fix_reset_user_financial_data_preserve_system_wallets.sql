...
begin
  ...
  order by
    case when coalesce(a.is_system, false) then 1 else 0 end,
  ...
  if array_length(deletable_account_ids, 1) is not null then
    ...
    where t.from_account_id = any(deletable_account_ids)
...
comment on function public.reset_user_financial_data() is
  'Resets user financial data while preserving system wallets and keeping one personal default wallet when available.';
