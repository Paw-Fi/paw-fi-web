CREATE OR REPLACE FUNCTION public.get_user_transactions_page_v4(
  p_user_id UUID,
...
BEGIN
  ...
  v_payload := public.get_user_transactions_page_v3(
    p_user_id,
    p_household_id,
    p_currency,
    p_currencies,
    ...
    jsonb_agg(
      item.value || jsonb_build_object(
        ...
        CASE
          ...
          THEN (
            SELECT expense.parent_recurring_id
            FROM public.expenses expense
            WHERE expense.id = (item.value ->> 'id')::UUID
          )
          ELSE NULL
        END
      )
...
REVOKE ALL ON FUNCTION public.get_user_transactions_page_v4(
  UUID, UUID, TEXT, TEXT[], TEXT, UUID, BOOLEAN, TEXT[], TEXT, TEXT,
  DATE, DATE, INTEGER, DATE, TIMESTAMPTZ, TEXT
) FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.get_user_transactions_page_v4(
  UUID, UUID, TEXT, TEXT[], TEXT, UUID, BOOLEAN, TEXT[], TEXT, TEXT,
  DATE, DATE, INTEGER, DATE, TIMESTAMPTZ, TEXT
) TO authenticated;

CREATE OR REPLACE FUNCTION public.get_mobile_delta_v4(
  p_user_id UUID,
...
BEGIN
  ...
  v_payload := public.get_mobile_delta_v3(
    p_user_id,
    p_since,
    p_since_id,
    p_limit
...
REVOKE ALL ON FUNCTION public.get_mobile_delta_v4(
  UUID, TIMESTAMPTZ, UUID, INTEGER
) FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.get_mobile_delta_v4(
  UUID, TIMESTAMPTZ, UUID, INTEGER
) TO authenticated;
