...
-- Policy: Users can view their own profiles OR service role can access all profiles (including guest profiles)
create policy "Users can view their own financial health profiles or service role access"
on public.financial_health_profiles for select
using (
  auth.uid() = user_id 
  OR auth.jwt() ->> 'role' = 'service_role'
  OR user_id IS NULL  -- Allow access to guest profiles
);

-- Policy: Users can insert their own profiles OR service role can insert any profile (including guest profiles)
create policy "Users can insert their own financial health profiles or service role access"
on public.financial_health_profiles for insert
with check (
  auth.uid() = user_id 
  OR auth.jwt() ->> 'role' = 'service_role'
  OR (user_id IS NULL AND auth.jwt() ->> 'role' = 'service_role') -- Allow service role to create guest profiles
);

-- Policy: Users can update their own profiles OR service role can update any profile
create policy "Users can update their own financial health profiles or service role access"
on public.financial_health_profiles for update
using (
  auth.uid() = user_id 
  OR auth.jwt() ->> 'role' = 'service_role'
  OR (user_id IS NULL AND auth.jwt() ->> 'role' = 'service_role') -- Allow service role to update guest profiles
);

-- Policy: Allow service role to delete any profile (for cleanup)
create policy "Service role can delete any financial health profile"
on public.financial_health_profiles for delete
...
comment on table public.financial_health_profiles is 'Stores AI-generated financial profiles based on user quiz responses. Supports both authenticated users and guest users (user_id can be null)';
comment on column public.financial_health_profiles.user_id is 'References the user who completed the quiz. Can be null for guest users';
