...
alter table public.budget_envelopes
  ...
  add column if not exists rollover_negative boolean not null default false,
  add column if not exists rollover_cap_cents bigint check (
    rollover_cap_cents is null or rollover_cap_cents >= 0
  ),
  add column if not exists opening_rollover_cents bigint not null default 0,
  add column if not exists rollover_group_id uuid;

with grouped_envelopes as (
  select
    e.user_id,
    e.household_id,
    upper(coalesce(nullif(trim(e.currency), ''), 'USD')) as currency,
...
set rollover_group_id = g.group_id
from grouped_envelopes g
where e.rollover_group_id is null
  and e.user_id = g.user_id
  and e.household_id is not distinct from g.household_id
  and upper(coalesce(nullif(trim(e.currency), ''), 'USD')) = g.currency
  and lower(trim(coalesce(e.name, ''))) = g.normalized_name;

alter table public.budget_envelopes
  alter column rollover_group_id set default gen_random_uuid(),
  alter column rollover_group_id set not null;

comment on column public.budget_envelopes.rollover_enabled is
  'When true, unused available pocket budget carries into the next matching monthly pocket.';
comment on column public.budget_envelopes.rollover_negative is
  'When true, overspending carries forward as negative rollover.';
comment on column public.budget_envelopes.rollover_cap_cents is
  'Optional maximum positive rollover amount in cents. Null means unlimited.';
comment on column public.budget_envelopes.opening_rollover_cents is
  'Manual rollover seed for this pocket month, useful when migrating from another budget app.';
comment on column public.budget_envelopes.rollover_group_id is
  'Stable lineage identifier shared by monthly copies of the same pocket; rollover history uses this instead of the display name.';

create index if not exists idx_budget_envelopes_rollover_lookup
  on public.budget_envelopes(rollover_group_id, currency)
  where rollover_enabled = true;

create index if not exists idx_budget_envelopes_rollover_name_fallback
...
begin
  ...
  loop
    ...
    if v_env_id is null or v_enabled = false then
      ...
      if v_cap is not null and v_carry > v_cap then
        v_carry := v_cap;
      end if;
...
begin
  ...
  display_rows as (
    select
      c.*,
      case
        when c.rollover_enabled and c.rollover_cap_cents is not null and c.incoming_rollover_cents > c.rollover_cap_cents
          then c.rollover_cap_cents
        when c.rollover_enabled then c.incoming_rollover_cents
        else 0
      end::bigint as display_rollover_cents
    from calculated c
  )
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'period_month', dr.period_month,
        'name', dr.name,
        'base_budget_cents', dr.base_cents,
        'rollover_from_previous_cents', dr.display_rollover_cents,
        'opening_rollover_cents', dr.opening_rollover_cents,
        'available_budget_cents',
          case
            when dr.rollover_enabled then dr.base_cents + dr.display_rollover_cents + dr.opening_rollover_cents
            else dr.base_cents
          end,
        'spent_cents', dr.spent_cents,
        'remaining_cents',
          case
            when dr.rollover_enabled then dr.base_cents + dr.display_rollover_cents + dr.opening_rollover_cents - dr.spent_cents
            else dr.base_cents - dr.spent_cents
          end,
        'rollover_enabled', dr.rollover_enabled,
        'rollover_negative', dr.rollover_negative,
        'rollover_cap_cents', dr.rollover_cap_cents
      )
      order by dr.period_month asc
    ),
    '[]'::jsonb
  )
  into v_rows
      ...
      jsonb_agg(
        re.row || jsonb_build_object(
          ...
          'rollover_from_previous_cents',
            case
              when re.rollover_enabled and re.rollover_cap_cents is not null and re.incoming_rollover_cents > re.rollover_cap_cents
                then re.rollover_cap_cents
              when re.rollover_enabled then re.incoming_rollover_cents
              else 0
            end,
          'available_budget_cents',
            case
              when re.rollover_enabled then
                re.base_cents +
                case
                  when re.rollover_cap_cents is not null and re.incoming_rollover_cents > re.rollover_cap_cents
                    then re.rollover_cap_cents
                  else re.incoming_rollover_cents
                end +
                re.opening_rollover_cents
              else re.base_cents
            end,
