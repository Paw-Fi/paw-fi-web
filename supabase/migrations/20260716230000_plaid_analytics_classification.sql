...
with plaid_classifications as (
  select
    e.id,
    upper(nullif(trim(e.raw_provider_payload #>> '{personal_finance_category,primary}'), '')) as provider_pfc_primary,
    upper(nullif(trim(e.raw_provider_payload #>> '{personal_finance_category,detailed}'), '')) as provider_pfc_detailed,
    upper(nullif(trim(e.raw_provider_payload #>> '{personal_finance_category,confidence_level}'), '')) as provider_pfc_confidence,
    lower(coalesce(nullif(trim(e.raw_provider_payload #>> '{personal_finance_category,version}'), ''), 'v1')) as provider_pfc_version,
    lower(nullif(trim(e.raw_provider_payload ->> 'transaction_code'), '')) as provider_transaction_code,
    case
      when jsonb_typeof(e.raw_provider_payload -> 'pending') = 'boolean'
        then (e.raw_provider_payload ->> 'pending')::boolean
      else false
    end as provider_pending,
    classification.*
  from public.expenses e
  join public.bank_accounts ba on ba.id = e.bank_account_id
...
set
  provider_pfc_primary = classification.provider_pfc_primary,
  provider_pfc_detailed = classification.provider_pfc_detailed,
  provider_pfc_confidence = classification.provider_pfc_confidence,
  provider_pfc_version = classification.provider_pfc_version,
  provider_transaction_code = classification.provider_transaction_code,
  provider_pending = classification.provider_pending,
  ...
  classification_source = 'plaid_pfc_' || classification.provider_pfc_version,
  classification_version = 2
from plaid_classifications classification
where classification.id = e.id;
