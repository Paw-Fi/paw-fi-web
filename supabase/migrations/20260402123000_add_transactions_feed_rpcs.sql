create index if not exists expenses_user_feed_v1_idx
  on public.expenses (user_id, date desc, created_at desc, id)
  where household_id is null and coalesce(is_recurring, false) = false;

create index if not exists expenses_household_feed_v1_idx
  on public.expenses (household_id, date desc, created_at desc, id)
  where coalesce(is_recurring, false) = false;

create index if not exists user_contacts_user_id_feed_v1_idx
  on public.user_contacts (user_id, updated_at desc, created_at desc, id);
    ...
    'category_summaries', coalesce(
      (
        select jsonb_agg(
          ...
          order by c.amount_cents desc, c.category asc
        )
