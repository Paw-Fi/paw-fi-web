...
-- Optimize cold Wallet Details transaction feed reads.
--
-- Wallet Details always calls the transaction feed RPCs with a concrete
-- p_account_id and p_include_unassigned_account = false. The existing RPCs are
-- intentionally generic, but that shape leaves broad OR predicates in the hot
-- wallet-detail path. This migration preserves the public RPC signatures and
-- response JSON, adds targeted feed-order indexes, and adds an account-specific
-- fast path before falling back to the previous general implementation.

create index if not exists idx_expenses_wallet_feed_account_currency_date
  on public.expenses (
    account_id,
...
create index if not exists idx_account_transfers_wallet_feed_from_currency_date
  on public.account_transfers (
    from_account_id,
    (upper(coalesce(currency, ''))),
    date desc,
    created_at desc,
    (('transfer:' || id::text || ':out')) desc
  );

create index if not exists idx_account_transfers_wallet_feed_to_currency_date
  on public.account_transfers (
    to_account_id,
    (upper(coalesce(currency, ''))),
    date desc,
    created_at desc,
    (('transfer:' || id::text || ':in')) desc
  );

-- Wallet Details recurring rows are loaded separately by recurringTransactionsProvider
-- and projected app-side into both the detail list and month totals. These indexes
-- keep that recurring-template load fast without returning template rows from the
-- transaction feed RPCs, which would double count/render them.
create index if not exists idx_expenses_recurring_personal_wallet_details
  on public.expenses (user_id, date desc, id)
...
create index if not exists idx_expenses_recurring_household_wallet_details
  on public.expenses (household_id, date desc, id)
  where is_recurring = true
...
-- Pocket Details fetches current-month expense rows by scope, currency, linked
-- categories, and date range. These expression indexes match the existing RPC
-- filters without changing the feed behavior or moving recurring projection into
-- the transaction feed.
create index if not exists idx_expenses_pocket_details_personal_currency_category_date
  on public.expenses (
    user_id,
    (upper(coalesce(currency, ''))),
...
create index if not exists idx_expenses_pocket_details_household_currency_category_date
  on public.expenses (
    household_id,
    (upper(coalesce(currency, ''))),
    (lower(coalesce(category, 'uncategorized'))),
