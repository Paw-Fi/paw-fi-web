...
function pickDeterministicIndex(opts: {
  recipientKey: string;
  dayKey: string;
  namespace: string;
  poolLength: number;
...
function buildNotificationMessage(
  ...
  switch (eventType) {
    ...
    case 'log_expense_reminder': {
      ...
      // Something stable per user/device. Pick whatever is available in payload.
      const recipientKey = String(
        payload.user_id ?? payload.device_id ?? payload.installation_id ?? 'anon'
      );

      // Optional forced variant for testing
      ...
      // Use server-local calendar day key
      ...
      const body = poolBody[bodyIndex];
      const title = poolTitle[titleIndex];
