...
serve(async (req: Request) => {
  ...
  if (req.method === "GET" && url.pathname.endsWith("/health")) {
    return new Response(
      JSON.stringify({
        ok: true,
        service: "households-send-push-notification",
  ...
  try {
    ...
    if (!isImmediateTransactionEvent && prefs && !prefs.enable_nudges) {
      ...
      return new Response(
        JSON.stringify({
          success: true,
          skipped: "User disabled notifications",
        }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (!isImmediateTransactionEvent) {
      if (isLogExpenseReminder) {
        ...
        const quietStart = Number.isFinite(Number(payload.quiet_start))
          ? Number(payload.quiet_start)
          : 22;
        const quietEnd = Number.isFinite(Number(payload.quiet_end))
          ? Number(payload.quiet_end)
          : 8;
        ...
        if (isInQuietHours(localMinutes, quietStart, quietEnd)) {
          console.log(
            "[send-push] Log expense reminder in quiet hours, will retry later",
          );
          await supabase
            .from("notification_events")
            .update({
    ...
    if (devicesError || !devices || devices.length === 0) {
      ...
      const inactiveCount = existingDevices?.length
        ? existingDevices.filter((d: any) => d && d.is_active === false).length
        : 0;
      console.log(
        "[send-push] No active devices found for user:",
        user_id,
        "existing:",
    ...
    if (
      event_type === "expense_added" &&
      (payload.expense_id || payload?.expense_data?.expense_id)
    ) {
      const expId = (payload.expense_id ||
...
function buildNotificationMessage(
  ...
  switch (eventType) {
    ...
    case "split_settled": {
      ...
      const code = payload.currency as string | undefined;
      const symbol = getCurrencySymbol(code);
      const youOweCents = Number(
        payload.amounts_before?.you_owe_cents ?? payload.amount_cents ?? 0,
      );
      const youAreOwedCents = Number(
        payload.amounts_before?.you_are_owed_cents ?? 0,
      );
      const netPayCents = Number(
        payload.amounts_before?.net_pay_cents ??
          Math.max(youOweCents - youAreOwedCents, 0),
      );
    ...
    case "invite_reminder_inviter": {
      const inviteeName = (payload.invitee_name ||
        payload.invited_email ||
        "Your invitee") as string;
      const title = "Pending Invitation";
      const body = `${inviteeName} hasn't joined ${householdName} yet`;
    ...
    case "invite_reminder_invitee": {
      const inviterName = (payload.inviter_name || "Someone") as string;
      const reminderTier = Number(payload.reminder_tier ?? 1);
      ...
      if (reminderTier === 3) {
        ...
        body = `Your invitation to ${householdName} expires in 2 days`;
      } else if (reminderTier === 2) {
        // Tier 2: Follow-up reminder - stronger nudge
        title = `${inviterName} is waiting`;
        body = `Accept your invitation to ${householdName} on Moneko`;
      } else {
        // Tier 1: Gentle reminder
        title = `Invitation from ${inviterName}`;
        body = `Join ${householdName} on Moneko to track finances together`;
      }
