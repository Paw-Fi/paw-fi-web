...
export function classificationHasNotificationEvidence(
  ...
  const currencyEvidence = classification.currencyEvidenceRaw?.trim() ||
    classification.currency;
  if (!normalizedContent.includes(currencyEvidence.toLocaleLowerCase())) {
    return false;
  }
...
Bare currency symbols may be ambiguous. Preserve the exact currency evidence and mark currencyAmbiguous when needed.
