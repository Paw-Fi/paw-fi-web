...
async function retryOperation<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = error;
      console.warn(
        `[retry] ${operationName} attempt ${attempt}/${maxRetries} failed:`,
        error,
      );
...
export async function processFreeFormTextExpense(params: {
      ...
      function_declarations: [
        {
          name: "set_budget",
          description:
            "Set or update the daily budget for this contact (today by default). Amount in major units.",
          parameters: {
            type: "OBJECT",
            properties: {
              amount: {
                type: "NUMBER",
                description: "Budget amount in major units",
                nullable: false,
              },
              currency: {
                type: "STRING",
        ...
        },
        {
          name: "add_expenses",
          description:
            "Append one or more expenses for this contact (today by default). Amount in major units.",
          parameters: {
            type: "OBJECT",
            properties: {
              items: {
  ...
  if (tool) {
    if (tool.name === "set_budget") {
      ...
      const currency =
        normalizeCurrencyCode(tool.args?.currency) || callerCurrency;
    ...
    if (tool.name === "add_expenses") {
      const items: ExpenseItem[] = Array.isArray(tool.args?.items)
        ? tool.args.items
        : [];
      ...
      const composed = items
        .map((it: any) => {
          const normalized =
            normalizeCurrencyCode(it.currency) || callerCurrency;
      ...
      try {
        ...
        error = e;
        console.error(
          "add_expenses via finance-update error (after retries):",
          error,
        );
        return {
          type: "expense",
  ...
  if (tool) {
    ...
    if (tool.name === "add_expenses") {
      const items: ExpenseItem[] = Array.isArray(tool.args?.items)
        ...
        : [];
      console.log(
        "[add_expenses] Received items from Gemini:",
