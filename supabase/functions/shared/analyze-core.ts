...
export function normalizeTransactionTime(value: unknown): string | undefined {
  ...
  const twelveHour =
    /^(\d{1,2})(?::(\d{2})(?::(\d{2}))?)?\s*([ap])\.?m\.?$/i.exec(trimmed);
  if (twelveHour) {
    ...
    const hour =
      (rawHour % 12) + (twelveHour[4].toLowerCase() === "p" ? 12 : 0);
...
export function extractExplicitTransactionTime(
  ...
  const labeledLine = sourceText
    .split(/\r?\n/)
    .find((line) =>
      /^\s*(?:date\s*(?:&|and|\/)\s*time|transaction\s*(?:date\s*(?:&|and|\/)\s*)?time|purchase\s*time|payment\s*time|time)\s*:/i.test(
        line,
      ),
    );
...
export function extractLabeledTransactionFallback(params: {
  ...
  const rawDate = params.sourceText
    ...
    ?.trim();
  const from =
    params.sourceText.match(/^\s*from\s*:\s*(.+)$/im)?.[1]?.trim() || "";
  const to = params.sourceText.match(/^\s*to\s*:\s*(.+)$/im)?.[1]?.trim() || "";
  const isIncome =
    /\b(my|your)\s+(account|a\/c)\b/i.test(to) &&
...
export function validateTransactionSourceGrounding(params: {
  ...
  const itemCurrency =
    typeof params.item.currency === "string"
  ...
  const merchant =
    typeof params.item.merchant === "string"
  ...
  if (merchant && !normalizedSource.includes(merchant)) {
    ...
    const description =
      typeof params.item.description === "string"
...
Return JSON only: {"decision":"APPROVE"|"REJECT","evidence":["exact source fragment"]}.`;
  ...
  async function verifyWithModel(modelName: string): Promise<boolean> {
    ...
    console.log("[HouseholdDefaultSplitDecisionTrace]", {
      ...
      source: source.kind,
      decision:
        decision === "APPROVE" || decision === "REJECT" ? decision : "INVALID",
  ...
  return rawItems
    .map((it) => {
      ...
      return {
        ...
        transactionTime:
          (shouldUseSourceText && sourceText
