...
Deno.serve(async (req: Request) => {
  ...
  const { data, error } = await supabase.rpc(
    "skip_next_installment_occurrence",
    {
  ...
  if (error) return mapRpcError(error, "Failed to skip installment occurrence");

  return jsonResponse({
    success: true,
    skippedOccurrenceId: (data as Record<string, unknown>)?.skippedOccurrenceId,
    replacementOccurrence: (data as Record<string, unknown>)
