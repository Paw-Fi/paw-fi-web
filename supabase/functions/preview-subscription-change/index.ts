...
serve(async (req) => {
  try {
    ...
    if (newPlan === 'free') {
      ...
      const periodEnd = new Date((subscription as any).current_period_end || Date.now()).toLocaleDateString()
      return new Response(JSON.stringify({
    ...
    try {
      ...
      const prorationLineItems = upcomingInvoice.lines.data.filter(line => line.proration)
      const totalProration = prorationLineItems.reduce((sum, line) => sum + line.amount, 0)
      
      // Get the new recurring amount
      const recurringLineItems = upcomingInvoice.lines.data.filter(line => !line.proration)
      const newRecurringAmount = recurringLineItems.reduce((sum, line) => sum + line.amount, 0)
      ...
      if (isUpgrade) {
        ...
        if (currentInterval !== newBillingInterval) {
          ...
          message = 'No changes needed - you are already on this plan and billing interval.'
        }
      }
      ...
      return new Response(JSON.stringify({
        ...
        newBillingInterval,
        billingBehavior,
        immediateCharge: billingBehavior === 'immediate' ? upcomingInvoice.amount_due : 0,
