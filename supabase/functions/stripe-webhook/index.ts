...
function mapStripeStatusToStoredStatus(
  status: string,
  ...
  switch (status) {
    ...
    case "incomplete_expired":
      return "canceled";
    default:
      console.warn("Unknown Stripe subscription status, treating as canceled", {
        status,
      });
      return "canceled";
  }
...
async function revokeLifetimeAccess(params: {
  ...
  if (existing?.stripe_customer_id) {
    const { data: lifetimeSource } = await supabase
    ...
    if (lifetimeSource?.status === "canceled") {
      return;
    }
  }
  ...
  try {
    ...
    console.error("Error revoking lifetime entitlement source:", updateError);
    return;
  }

  // CRITICAL: Cascade cancellation to all household members bound to this lifetime subscription
  if (
    previousProjection?.status === "active" ||
    previousProjection?.status === "trialing"
  ) {
    try {
...
async function handleSubscriptionUpdated(
  ...
  try {
    ...
    let primaryProjection:
      | ReturnType<typeof mapStripeSubscriptionToSource>
      | null = null;

    try {
      const stripeSource = mapStripeSubscriptionToSource({
        userId,
        subscription,
      ...
      primaryProjection = projectionResult.primary;
    } catch (subscriptionError) {
      console.error(
        "Error updating subscription entitlement source:",
        subscriptionError,
      );
      return;
    }

    const isCurrentPrimaryStripe = primaryProjection?.provider === "stripe" &&
      primaryProjection.stripeSubscriptionId === subscription.id;

    if (!isCurrentPrimaryStripe) {
      console.log(
        "Stored Stripe source update without changing primary projection",
        {
          userId: redactUserId(userId),
          subscriptionId: subscription.id,
          primaryProvider: primaryProjection?.provider ?? null,
        },
      );
      return;
    }
    ...
    if (isTerminalDowngradeStatus(status)) {
      console.log(
        `Subscription ${subscription.id} entered terminal status ${status}, downgrading projected access`,
      );
...
async function handleSubscriptionDeleted(
  ...
  try {
    ...
    const sourceKey = buildStripeSubscriptionSourceKey(subscription.id);
    const { data: sourceData, error: sourceError } = await supabase
      .from("subscription_entitlement_sources")
      .select("user_id, plan, status, previous_plan, previous_interval")
      .eq("provider", "stripe")
      .eq("source_key", sourceKey)
      .maybeSingle();

    if (sourceError) {
      console.error("Error finding subscription source:", sourceError);
      return;
    }

    const fallbackProjection = !sourceData?.user_id
      ? await supabase
        .from("subscriptions")
    ...
    const resolvedUserId = (sourceData?.user_id as string | null) ??
      (fallbackProjection?.data?.user_id as string | null) ??
      null;
    const resolvedPlan = (sourceData?.plan as PlanType | null) ??
      (fallbackProjection?.data?.plan as PlanType | null) ??
      null;
    const resolvedStatus = (sourceData?.status as string | null) ??
      (fallbackProjection?.data?.status as string | null) ??
      null;
    ...
    if (resolvedPlan === "lifetime" && resolvedStatus === "active") {
      console.log(
    ...
    const canceledAtIso = typeof subscription.ended_at === "number" &&
    ...
    const priceId = subscription.items.data[0]?.price?.id;
    const planInfo = getPlanFromPriceId(priceId);
    const plan = resolvedPlan ?? planInfo?.plan ?? "plus";
    const billingInterval = planInfo?.interval ??
    ...
    const { data: previousProjection } = await supabase
      .from("subscriptions")
    ...
    await upsertSubscriptionEntitlementSource({
      ...
      source: {
        ...
        status: "canceled",
        billingInterval,
        currentPeriodEnd: canceledAtIso,
        cancelAtPeriodEnd: false,
        trialStart: typeof subscription.trial_start === "number" &&
            !Number.isNaN(subscription.trial_start)
          ? new Date(subscription.trial_start * 1000).toISOString()
          : null,
        trialEnd: typeof subscription.trial_end === "number" &&
            !Number.isNaN(subscription.trial_end)
          ? new Date(subscription.trial_end * 1000).toISOString()
          : null,
        stripeCustomerId: customerId,
        ...
        previousInterval: null,
        lastEventId: eventId,
        createdAt: typeof subscription.created === "number"
          ? new Date(subscription.created * 1000).toISOString()
          : canceledAtIso,
        updatedAt: canceledAtIso,
      },
    });
    ...
    const primaryStillGrantsAccess = projectionResult.primary &&
      (projectionResult.primary.status === "active" ||
        projectionResult.primary.status === "trialing");

    if (!primaryStillGrantsAccess) {
      try {
        await supabase.rpc("cascade_subscription_cancellation", {
          p_owner_user_id: userId,
        });
        ...
        reportStripeWebhookError(
          "handle_subscription_deleted_cascade",
          cascadeError,
          {
            userId,
            subscriptionId: subscription.id,
...
async function handleInvoicePaymentFailed(
  ...
  try {
    ...
    const sourceKey = buildStripeSubscriptionSourceKey(subscriptionId);
    const { data: sourceData, error: sourceError } = await supabase
      .from("subscription_entitlement_sources")
    ...
    if (sourceError) {
      console.error(
        "Error finding Stripe source for invoice payment failure:",
        sourceError,
      );
      return;
    }
    ...
    const userId = (sourceData?.user_id as string | null) ??
      (fallbackProjection?.data?.user_id as string | null) ??
      null;
    const plan = (sourceData?.plan as string | null) ??
      (fallbackProjection?.data?.plan as string | null) ??
      null;

    if (!userId) {
      console.error(
        "No Stripe source found with subscription ID:",
        subscriptionId,
      );
      return;
    }

    const sourceUpdateEventId = `invoice_payment_failed_${invoice.id}`;
    ...
    if (latestSubscription) {
      const stripeCustomerId =
        (sourceData?.stripe_customer_id as string | null) ??
          (fallbackProjection?.data?.stripe_customer_id as string | null) ??
          (typeof latestSubscription.customer === "string"
            ? latestSubscription.customer
            : latestSubscription.customer?.id ?? null);
      ...
      try {
        ...
        await upsertSubscriptionEntitlementSource({
          ...
          source: {
            ...stripeSource,
            status: mappedStatus,
            lastEventId: sourceUpdateEventId,
            updatedAt: new Date().toISOString(),
          },
        });
        ...
        const isCurrentPrimaryStripe =
          projectionResult.primary?.provider === "stripe" &&
          projectionResult.primary.stripeSubscriptionId === subscriptionId;

        if (
          isCurrentPrimaryStripe &&
          isTerminalDowngradeStatus(latestStripeStatus)
        ) {
          await downgradeOwnerSubscriptionToFree({
            userId,
            eventId: sourceUpdateEventId,
            status: mappedStatus === "unpaid" ? "unpaid" : "canceled",
          });
          console.log(
            `Subscription moved to terminal state ${latestStripeStatus}; projected access downgraded`,
          );
        } else if (isCurrentPrimaryStripe && plan) {
          try {
        ...
        reportStripeWebhookError(
          "handle_invoice_payment_failed_source_update",
          sourceUpdateError,
          {
            invoiceId: invoice.id,
            subscriptionId,
        ...
        console.error(
          "Failed to reconcile Stripe source after invoice payment failure",
          {
            subscriptionId,
            userId,
            error: sourceUpdateError instanceof Error
              ? sourceUpdateError.message
              : String(sourceUpdateError),
          },
        );
      }
    }
  ...
  try {
    ...
    if (session.mode === "subscription") {
      ...
      if (plan === "lifetime") {
        ...
        const lifetimeNowIso = new Date().toISOString();
        ...
        try {
          await upsertSubscriptionEntitlementSource({
            ...
            source: {
              ...
              previousInterval: null,
              lastEventId: eventId,
              createdAt: lifetimeNowIso,
              updatedAt: lifetimeNowIso,
            },
          });
          await recomputeProjectedSubscription({
            supabase,
