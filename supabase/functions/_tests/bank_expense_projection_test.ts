...
import type { ExpenseUpsertInput, PlaidTransaction } from "../shared/plaid-client.ts";
import {
  buildBankExpenseMutationPlan,
  computeBankExpenseUserOverrides,
} from "../shared/bank-expense-projection.ts";

function makeExpenseRecord(overrides: Partial<ExpenseUpsertInput> = {}): ExpenseUpsertInput {
  return {
    user_id: "user-1",
...
Deno.test("bank expense projection preserves user overrides on provider updates", () => {
...
Deno.test("bank expense projection merges pending transaction into posted row update", () => {
  const plan = buildBankExpenseMutationPlan({
    records: [
      makeExpenseRecord({
...
Deno.test("bank expense projection undeletes provider-removed transactions on re-add", () => {
  const plan = buildBankExpenseMutationPlan({
...
Deno.test("bank expense projection keeps user-deleted transactions hidden", () => {
  const plan = buildBankExpenseMutationPlan({
    records: [makeExpenseRecord()],
    transactions: [makePlaidTransaction()],
    existingRows: [
