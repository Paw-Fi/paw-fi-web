...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (
      (!candidate && !internalMerchant) ||
      (body.action !== "resolve" && !transactionId)
    ) {
      return json({ error: "Selected merchant is invalid" }, 400);
    }
    ...
    // New transactions do not yet have an expense ID. Resolve the candidate
    // first, then let the transaction's own local-first save persist it.
    if (body.action === "resolve") {
      return json({ success: true, merchant, allowStructuredLearning });
    }
