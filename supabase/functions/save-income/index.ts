...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!['private', 'balances_only', 'full'].includes(privacyScope)) {
      return new Response(
        JSON.stringify({ error: 'Invalid privacy scope. Must be: private, balances_only, or full' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (!['me', 'partner', 'household'].includes(ownerType)) {
      return new Response(
        JSON.stringify({ error: 'Invalid owner type. Must be: me, partner, or household' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (contactError) {
      console.error('[save-income] Failed to look up user contact:', contactError);
      return new Response(
    ...
    if (body.householdId) {
      incomeRecord.household_id = body.householdId;
    ...
    if (body.householdId && body.idempotencyKey) {
      ...
      if (existing) {
        console.log('[save-income] Duplicate detected (idempotency), returning existing:', existing.id);
    ...
    if (incomeError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to save income', details: incomeError.message }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    // If household income, create notification for household members
    if (body.householdId) {
      console.log('[save-income] Creating household notifications for income:', body.householdId);
      ...
      if (!membership) {
        console.warn('[save-income] User is not a member of household:', body.householdId);
        return new Response(
          JSON.stringify({
            success: true,
            data: income,
            warning: 'Income saved but not shared (not a household member)',
          }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      if (notifyError) {
        console.error('[save-income] Error creating notifications:', notifyError);
        // Don't fail the request, just log the error
      } else {
        console.log('[save-income] Notifications created for household members');
      }
    }
