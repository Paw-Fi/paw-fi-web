...
function checkProfileCompleteness(userProfile: any): {
  isComplete: boolean;
  isPartial: boolean;
  ...
  const completionRate = Math.round(
    (completedFields / essentialFields.length) * 100,
  );
...
function createTimestampPair(): { userTimestamp: string; aiTimestamp: string } {
  ...
  return {
    ...
    aiTimestamp: new Date(
      baseTimestamp.getTime() + MESSAGE_TIMESTAMP_OFFSET_MS,
    ).toISOString(),
  };
}
...
`;
  ...
  goalsSummary.forEach((goal: any, index: number) => {
    const progressStatus = goal.is_on_track
      ? "✅ ON TRACK"
      : "⚠️ BEHIND SCHEDULE";
    const completionDate = goal.target_date
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!currentConversationId) {
      ...
      console.log(
        `Created new chat session: ${currentConversationId} for ${userId ? "authenticated user" : "guest"}`,
      );
    }
    ...
    if (userId) {
      const { data: userActivitiesData, error: activitiesError } =
        await supabase.functions.invoke(`user-activities?user_id=${userId}`, {
          method: "GET",
        });
      if (activitiesError) {
        console.error("Error fetching user activities:", activitiesError);
      } else userActivities = userActivitiesData?.activities;
    }
    ...
    if (userId) {
      ...
      if (!userProfile || !profileStatus.isComplete) {
        ...
        if (!userProfile) {
          ...
          if (isFirstMessage) {
            ...
            if (chatModel === AI_ROLES.FINANCIAL_ADVISOR) {
              aiResponse = getRandomResponse(
                RANDOM_RESPONSES.FINANCIAL_ADVISOR_FIRST,
              );
            } else {
            ...
            if (chatModel === AI_ROLES.FINANCIAL_ADVISOR) {
              aiResponse = getRandomResponse(
                RANDOM_RESPONSES.FINANCIAL_ADVISOR_FOLLOWUP,
              );
            } else {
              aiResponse = getRandomResponse(
                RANDOM_RESPONSES.EDUCATOR_FOLLOWUP,
              );
            }
          }
          ...
          let partialResponse;
          if (chatModel === AI_ROLES.FINANCIAL_ADVISOR) {
            partialResponse = getRandomResponse(
              RANDOM_RESPONSES.FINANCIAL_ADVISOR_PARTIAL_PROFILE,
            );
          } else {
            partialResponse = getRandomResponse(
              RANDOM_RESPONSES.EDUCATOR_PARTIAL_PROFILE,
            );
          }
          // Replace the completion rate placeholder
          aiResponse = partialResponse.replace(
            /{{completionRate}}/g,
            profileStatus.completionRate.toString(),
          );
        } else {
    ...
    if (!userId || userProfile) {
        ...
        try {
          ...
          if (goalTrackingResult.function_executed) {
            ...
            if (currentConversationId) {
              ...
              if (batchInsertError) {
                console.error(
                  "Error saving goal tracking messages:",
                  batchInsertError,
                );
              }
            }
            ...
            if (isNavigationRequest(message)) {
              ...
              if (currentConversationId) {
                ...
                if (batchInsertError) {
                  console.error(
                    "Error saving fallback messages:",
                    batchInsertError,
                  );
                }
              }
          ...
          console.error(
            "Goal tracking error, falling back to normal processing:",
            goalError,
          );
      ...
      if (
        chatModel === AI_ROLES.FINANCIAL_ADVISOR &&
        goalContext &&
        !goalTrackingAttempted
      ) {
      ...
      while (
        !isJsonComplete(responseText) &&
        attempts < MAX_JSON_COMPLETION_ATTEMPTS
      ) {
        attempts++;
    ...
    const hasEmbeddedJson =
      chatModel === AI_ROLES.FINANCIAL_EDUCATOR &&
    ...
    if (!hasJsonMarkdown && !looksLikeRawJson && !hasEmbeddedJson) {
      ...
      if (hasJsonMarkdown) {
        ...
        const lastSuffixIndex =
          potentialJsonContent.lastIndexOf(markdownSuffix);
        ...
        if (
          ...
          stringToParseForJson = cleanedResponse.substring(
            firstBraceIndex,
            lastBraceIndex + 1,
          );
        } else {
      ...
      try {
        ...
        if (extractedCourse) {
          ...
          try {
            ...
            try {
              parsedCourse =
                typeof extractedCourse === "string"
                  ? JSON.parse(extractedCourse)
                  : extractedCourse;
            } catch (err) {
              ...
              const { error, data } = await supabase.functions.invoke(
                "store-course-from-ai",
                {
                  body: {
                    ...
                    session_id: requestData.userId
                      ? null
                      : currentConversationId, // session_id for guests
                    course: parsedCourse,
                  },
                },
              );
        ...
        const parsedJson =
          typeof wellFormedJsonString === "string"
        ...
        const fullMessageParts = [preamble, courseCardHtml, epilogue].filter(
          Boolean,
        );
    ...
    if (currentConversationId) {
      ...
      const messagesToInsert = [
        ...
        },
        {
          ...
          role: "assistant",
          metadata: extractedCourse
            ? { courseRecommendation: extractedCourse }
            : null,
          timestamp: timestamps.aiTimestamp,
        },
      ];
    ...
    if (
      ...
      let fallbackResponse =
        "I apologize, but I didn't quite understand your request. ";

      if (chatModel === AI_ROLES.FINANCIAL_ADVISOR) {
        fallbackResponse +=
          "As your financial advisor, I'm here to help with financial planning, goal management, budgeting advice, and investment guidance. Could you please rephrase your question or let me know what specific financial topic you'd like assistance with?";
      } else {
        fallbackResponse +=
          "As your financial educator, I'm here to help you learn about personal finance topics, investment concepts, and money management strategies. What would you like to learn about today?";
      }
