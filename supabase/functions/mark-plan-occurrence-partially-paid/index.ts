...
Deno.serve(async (req: Request) => {
  ...
  const occurrenceId = requestBody.occurrenceId;
  ...
  if (typeof occurrenceId !== "string" || occurrenceId.trim().length === 0) {
    return errorResponse("occurrenceId is required", 400);
  }
  if (
  ...
  const { data, error } = await supabase.rpc("mark_plan_occurrence_paid", {
    p_actor_user_id: userId,
    p_plan_id: planId,
    p_occurrence_id: occurrenceId,
    p_amount_cents: amountCents,
    p_payment_date: paymentDate,
    p_payment_kind: "partial",
  ...
  if (error) return mapRpcError(error, "Failed to record partial payment");

  return jsonResponse({
    success: true,
    plan: (data as Record<string, unknown>)?.plan,
    occurrence: (data as Record<string, unknown>)?.occurrence,
  });
});
