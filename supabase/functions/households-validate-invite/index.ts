...
interface ValidateInviteResponse {
  ...
  error?: string;
  error_code?: 'EXPIRED' | 'REVOKED' | 'ACCEPTED' | 'NOT_FOUND' | 'ALREADY_MEMBER';
}
...
serve(async (req) => {
  ...
  try {
    ...
    // Get inviter details
    const { data: inviter } = await supabase.auth.admin.getUserById(invite.inviter_id);

    // Also fetch profile details from public.users
    ...
    const response: ValidateInviteResponse = {
      ...
      inviter: inviter?.user ? {
        ...
        full_name: inviterProfile?.full_name || inviter.user.user_metadata?.full_name || undefined,
        avatar_url: inviterProfile?.avatar_url || inviter.user.user_metadata?.avatar_url || null
      } : undefined,
      expires_at: invite.expires_at,
      status: invite.status,
      invite: {
