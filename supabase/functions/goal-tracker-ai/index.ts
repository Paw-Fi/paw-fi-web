...
import {
  logUserActivity,
  type ActivityData,
} from "../shared/activity-logger.ts";
...
function getParameterSchema(functionKey: string) {
  const baseSchema = {
    ...
    isGlobalMode: {
      type: "boolean",
      description: "Whether operating in global mode",
    },
  ...
  switch (functionKey) {
    case "update_progress":
      return {
        ...
        updateType: {
          type: "string",
          description:
            "Type of update: goal_progress_updated or milestone_completed",
        },
        milestoneId: {
          type: "string",
          description: "Milestone ID if completing milestone (optional)",
        },
      };
    ...
    case "create_goal":
      return {
        ...
        goalType: {
          type: "string",
          description:
            "Type of goal: emergency_fund, retirement, home_buying, wealth, investment, debt_payoff, or custom. Use 'custom' for travel, vacation, car, or other personal goals.",
        },
        questionnaireAnswers: {
          type: "object",
          description:
            "User's financial information including goalName, targetAmount, timeframe, monthlyIncome, monthlyExpenses, currentSavings, riskTolerance, financialPriorities",
        },
      };
    case "generate_insights":
      return {
        ...
        goalId: {
          type: "string",
          description: "Goal ID for insights (optional in global mode)",
        },
      };
    default:
      return baseSchema;
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log(
      `Processing goal tracker message for user: ${userId}, global mode: ${isGlobalMode}`,
    );
    ...
    if (functionCall) {
      ...
      let functionArgs = {
        ...
        goalId:
          functionCall.args.goalId || (goalContext || goal)?.goalId || goalId,
        userId: functionCall.args.userId || userId,
        isGlobalMode:
          functionCall.args.isGlobalMode !== undefined
            ? functionCall.args.isGlobalMode
            : isGlobalMode,
      };
      ...
      if (functionCall.name === "goal-progress-tracker") {
        functionArgs = {
          ...functionArgs,
          updateType:
            functionArgs.updateType || RewardActions.GOAL_PROGRESS_UPDATED, // Use RewardActions constant
          // Ensure we have required parameters
          goalId:
            functionArgs.goalId ||
            (isGlobalMode && goalContext?.goalsSummary?.[0]?.id),
        };
      }
      ...
      if (functionCall.name === "ai-goal-generator") {
        ...
        let goalType =
          functionArgs.goalType || functionArgs.payload?.goal_type || "custom";

        // Map unsupported goal types to supported ones
        const supportedGoalTypes: GoalType[] = [
          "emergency_fund",
          "retirement",
          "home_buying",
          "wealth",
        ...
        if (!supportedGoalTypes.includes(goalType)) {
          console.log(
            `Mapping unsupported goal type '${goalType}' to 'custom'`,
          );
          goalType = "custom";
        }
        ...
        if (targetDate) {
          ...
          if (isNaN(dateObj.getTime()) || dateObj <= minDate) {
            console.log(
              `Invalid target date '${targetDate}', using 1 year from now`,
            );
        ...
        const basicQuestionnaireAnswers = functionArgs.questionnaireAnswers || {
          ...
          monthly_contribution:
            ...
            Math.round((payload.target_amount || 10000) / 12), // Default to 12 months
          ...
          financialPriorities: [
            goalType === "custom" ? payload.goal_type || "travel" : goalType,
          ],
        };
        ...
        console.log(
          `Enhanced ai-goal-generator args:`,
          JSON.stringify(
            {
              goalType,
      ...
      return new Response(
        JSON.stringify({
          ...
          next_actions: generateNextActions(
            functionCall.name,
            functionResult.data,
          ),
