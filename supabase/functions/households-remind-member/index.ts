...
serve(async (req) => {
  ...
  try {
    ...
    if (!household_id || !target_user_id) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: household_id, target_user_id' }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    if (!senderMember || !targetMember) {
      return new Response(
        JSON.stringify({ error: 'Both users must be members of the household' }),
        {
          status: 403,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    // if (recentReminder) {
    //   const timeLeft = new Date(recentReminder.created_at);
    //   timeLeft.setHours(timeLeft.getHours() + 24);
    ...
    //       cooldown_ends_at: timeLeft.toISOString()
    //     }),
    //     {
    //       status: 429,
    //       headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    ...
    const senderName = senderUser?.full_name || senderUser?.email?.split('@')[0] || 'A member';
    ...
    if (notificationError) {
      console.error('[remind-member] Error creating notification:', notificationError);
      return new Response(
