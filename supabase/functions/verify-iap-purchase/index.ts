...
serve(async (req: Request) => {
  ...
  try {
    ...
    const body = safeJson<VerifyRequestBody>(await req.json().catch(() => null))
    ...
    if (!storeProductId) {
      return new Response(JSON.stringify({ error: 'storeProductId is required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    const billingInterval = (catalogProduct.billing_interval as BillingInterval | null) ?? null
    ...
    if (body.platform === 'ios') {
      ...
      const receiptData = asString(body.verificationData?.serverVerificationData)
      if (!receiptData) {
        return new Response(JSON.stringify({ error: 'Missing iOS receipt data' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
      ...
      if (receiptResponse.status !== 0) {
        ...
        return new Response(JSON.stringify({ error: 'Invalid receipt', appleStatus: receiptResponse.status }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
      ...
      if (plan === 'lifetime') {
        ...
        const inApp = (receiptResponse.receipt?.in_app ?? []) as Array<Record<string, unknown>>
        ...
        if (!latest) {
          return new Response(JSON.stringify({ error: 'Lifetime purchase not found in receipt' }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          })
        }
        ...
        if (latest['cancellation_date_ms']) {
          return new Response(JSON.stringify({ error: 'Purchase was refunded' }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          })
        }
        ...
        const latestInfo = (receiptResponse.latest_receipt_info ?? []) as Array<Record<string, unknown>>
        ...
        const latest = matching[0]
        if (!latest) {
          return new Response(JSON.stringify({ error: 'Subscription not found in receipt' }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          })
        }
        ...
        if (latest['cancellation_date_ms']) {
          return new Response(JSON.stringify({ error: 'Subscription was refunded' }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          })
        }
        ...
        if (!expiresIso) {
          return new Response(JSON.stringify({ error: 'Invalid subscription expiry' }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          })
        }
      ...
      if (!eventKey) {
        return new Response(JSON.stringify({ error: 'Missing Apple transaction identifiers' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
      ...
      if (idemError && idemError.code !== '23505') {
        ...
        return new Response(JSON.stringify({ error: 'Failed to process purchase (idempotency)' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
      ...
      return new Response(JSON.stringify({ verified: true, subscription: finalSub }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (!androidPackageName) {
      return new Response(JSON.stringify({ error: 'ANDROID_PACKAGE_NAME not configured' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }

    const purchaseToken = asString(body.verificationData?.serverVerificationData)
    if (!purchaseToken) {
      return new Response(JSON.stringify({ error: 'Missing Android purchase token' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (isLifetime) {
      ...
      if (!resp.ok || !data) {
        console.error('Google products.get failed:', { status: resp.status, data })
        return new Response(JSON.stringify({ error: 'Invalid purchase token' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
      ...
      const data = await resp.json().catch(() => null)
      if (!resp.ok || !data) {
        console.error('Google subscriptionsv2.get failed:', { status: resp.status, data })
        return new Response(JSON.stringify({ error: 'Invalid purchase token' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      ...
      if (!currentPeriodEnd) {
        ...
        return new Response(JSON.stringify({ error: 'Invalid subscription expiry from Google' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
