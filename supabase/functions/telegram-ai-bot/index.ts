...
const SYSTEM_INSTRUCTION = buildBotSystemInstruction("Telegram");
...
Deno.serve(async (req: Request) => {
  ...
  // Start before the reservation RPC so database latency never hides activity.
  const stopTypingHeartbeat = startTelegramTypingHeartbeat(
    TELEGRAM_BOT_TOKEN,
    chatId,
  );

  let reserve: Awaited<ReturnType<typeof reserveIdempotency>>;
  try {
    reserve = await reserveIdempotency(
      supabase,
      idempotencyKey,
      ...
      try {
        ...
        if (incomingText && isStartVerification(incomingText)) {
          ...
          const { error: clearVerificationError } = await supabase
            ...
            .eq("verified", false);
          if (clearVerificationError) throw clearVerificationError;

          const { error: createVerificationError } = await supabase
            .from("whatsapp_verifications")
            .insert({
              channel: "telegram",
          ...
          if (createVerificationError) throw createVerificationError;
          ...
          const sentVerification = await sendTelegramMessage(
            TELEGRAM_BOT_TOKEN,
            chatId,
            msg,
            {
          ...
          if (!sentVerification) {
            throw new Error("Failed to deliver Telegram verification message");
          }
        ...
        if (!contact || !contact.verified || !contact.user_id) {
          ...
          const sentVerificationPrompt = await sendTelegramMessage(
            TELEGRAM_BOT_TOKEN,
            chatId,
            prompt,
            {
          ...
          if (!sentVerificationPrompt) {
            throw new Error("Failed to deliver Telegram verification prompt");
          }
          telegramResponseSent = true;
          await updateIdempotency(supabase, idempotencyKey, {
            status: "done",
        ...
        let subscription = contextData
          ? {
            ...
            currentPeriodEnd: contextData.subscription_current_period_end ??
              null,
          }
          : null;
        ...
        if (!hasPlusEntitlement(subscription)) {
          const nonSubscriberMessage = buildSubscriptionRequiredMessage(
            "Telegram capture",
          );
          const sentSubscriptionRequired = await sendTelegramMessage(
            TELEGRAM_BOT_TOKEN,
            chatId,
          ...
          if (!sentSubscriptionRequired) {
            throw new Error("Failed to deliver Telegram subscription prompt");
          }
          telegramResponseSent = true;
          await updateIdempotency(supabase, idempotencyKey, {
            status: "done",
        ...
        const chatSpacesForPrompt = (chatHouseholds as any[]).map((h: any) => ({
          ...
          space_type: h?.is_portfolio === true
            ? "private_space"
            : "shared_space",
        }));
        ...
        if (!session) {
          ...
          if (sessionError || !newSession) {
            ...
            const sentSessionFailure = await sendTelegramMessage(
              TELEGRAM_BOT_TOKEN,
              chatId,
              "Failed to initialize chat session.",
            );
            if (!sentSessionFailure) {
              throw new Error("Failed to deliver Telegram session error");
            }
            telegramResponseSent = true;
            await updateIdempotency(supabase, idempotencyKey, {
              status: "failed",
        ...
        while (
          ...
          for (const call of functionCalls) {
            ...
            const executionToolName = call.name === "list_expenses" &&
            ...
            try {
              if (call.name === "analyze_expense") {
                ...
                const wantsPersonalOnly = !householdId &&
                  ["personal", "personal_account"].includes(normalizedScope);
                const wantsSharedOnly = !householdId &&
                  ["shared", "shared_space"].includes(normalizedScope);
                ...
                const splitConfig = householdId && !spaceMeta?.isPortfolio
                ...
                for (const [index, row] of rows.entries()) {
                  ...
                  const splitConfig = householdId && !isPortfolio
                    ? await resolveHouseholdSplitConfig(
                      supabase,
                      householdId,
                      userId,
                  ...
                  batchTransactions.push({
                    ...
                    accountId: requestedWallet.accountId ?? undefined,
                    source: row.source,
                    ownerType: row.owner_type === "space"
                      ? "household"
                      : row.owner_type || "me",
                ...
                toolResult = await executeBotFinancialInsight({
                  ...
                  args: call.name === "list_expenses"
                    ? buildFinancialInsightArgs(incomingText, call.args || {})
                    : call.args || {},
                  spaceMap,
                  logPrefix: "telegram-ai-bot",
                  chartRequested: isFinancialInsightChartRequested(
                    incomingText,
                  ),
                ...
                if (!updatesArgs) {
                  ...
                  if (!lastRead.items || lastRead.items.length === 0) {
                    ...
                    const { data: fallbackData, error: fallbackError } =
                      await fetchExpensesDirect(supabase, contact.id, {
                        ...
                        accessibleHouseholdIds: listBotSpaceIds(spaceMap),
                  ...
                  if ("needs_disambiguation" in resolved) {
                    ...
                    const hasScopeUpdate = hasExplicitBotSpaceScope(
                      updatesArgs,
                    );
                        ...
                        const currencyResult = resolveWalletTransactionCurrency(
                          {
                            ...
                            walletName: (updatesArgs as any).wallet_name ||
                              (updatesArgs as any).wallet_id ||
                              (updatesArgs as any).account_id,
                            transactionCurrency: updates.currency ||
                ...
                if (
                  ...
                  if (!amountResult.ok) {
                    ...
                    const periodMonth =
                      await resolveFinancialPeriodStartForUser(
                        supabase,
                        userId,
                        dateStr,
                      );
            ...
            if (typeof toolResult?.error === "string") {
              toolErrorTexts.push(toolResult.error);
            }
            ...
            if (succeeded) {
              ...
              if (isWriteMutationToolName(call.name, toolResult)) {
            ...
            const chartFromTool = extractChartMediaUrlFromToolResult(
              toolResult,
            );
            if (chartFromTool) {
        ...
        let mediaUrl: string | null = chartFromText.url ||
          lastGeneratedChartUrl;
        ...
        if (mediaUrl) {
          ...
          if (!sentMessage) {
            throw new Error("Failed to deliver Telegram response");
          }
          telegramResponseSent = true;
        }
