...
function ResetPassword() {
  ...
  useEffect(() => {
    const initializeRecoverySession = async () => {
      try {
        ...
        console.warn(
          "[ResetPassword] No session and no usable recovery parameters, marking link invalid",
        );
        setIsValidSession(false);
      } catch (err) {
        console.error(
          "[ResetPassword] Error initializing recovery session:",
          err,
        );
        setIsValidSession(false);
      }
  ...
  if (isValidSession === false) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50">
        <div className="mx-auto w-full max-w-md rounded-lg bg-white p-6 text-center shadow-md">
