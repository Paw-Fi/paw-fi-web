...
import { faCircleCheck, faCircleXmark, faCircleExclamation, faClock, faTriangleExclamation } from "@fortawesome/free-solid-svg-icons";
...
export const Route = createFileRoute("/payment-status")({
  validateSearch: (search: Record<string, unknown>): PaymentStatusSearchParams => {
    return {
...
function PaymentStatusPage() {
  ...
  const [paymentStatus, setPaymentStatus] = useState<string>(search.status || 'pending');
  ...
  useEffect(() => {
    const verifyPayment = async () => {
      ...
      if (search.session_id && paymentStatus === 'success') {
        try {
          setIsLoading(true);
          
          // Call the verify-payment function
          const { data, error } = await supabase.functions.invoke('verify-payment', {
            body: { sessionId: search.session_id },
          });
    ...
    verifyPayment();
  }, [search.session_id, paymentStatus, search.status]);
              ...
              <>
                        ...
                        <div className="mt-4 rounded-lg bg-white/50 p-4 dark:bg-slate-800/50">
                          ...
                          <ul className="space-y-2 text-sm">
                            ...
                            <li className="flex items-center justify-between">
                              ...
                              <span className="font-medium">{subscriptionDetails.cancelAtPeriodEnd ? 'No' : 'Yes'}</span>
                            </li>
                          </ul>
                        </div>
                      )}
                ...
                {paymentStatus === 'failed' && (
                  <Alert variant="destructive" className="mb-6">
                    <FontAwesomeIcon icon={faCircleXmark} className="h-5 w-5 text-red-500" />
                ...
                {paymentStatus === 'canceled' && (
                  <Alert className="mb-6">
                    <FontAwesomeIcon icon={faCircleExclamation} className="h-5 w-5 text-blue-500" />
                ...
                {!['success', 'failed', 'canceled', 'pending'].includes(paymentStatus) && (
                  <Alert variant="default" className="mb-6">
                    <FontAwesomeIcon icon={faTriangleExclamation} className="h-5 w-5 text-yellow-500" />
