...
export const Route = createFileRoute("/solutions/$categorySlug")({
  ...
  component: SolutionCategoryRoute,
});

function SolutionCategoryRoute() {
  const { category, guides } = Route.useLoaderData();
  const featuredGuides = category.featuredGuideSlugs
    .map((slug) => guides.find((guide) => guide.slug === slug))
    .filter((guide): guide is (typeof guides)[number] => Boolean(guide));
  const location = useLocation();
  const isCategoryPage = location.pathname === `/solutions/${category.slug}`;
              ...
              <aside className="space-y-8">
                ...
                <section className="border-border bg-card/70 rounded-3xl border p-6">
                  ...
                  <p className="text-muted-foreground mt-3 text-sm leading-relaxed">
                    This intro page is the hub for the category. It links to
                    every solution article and gives search engines a clear path
                    into the related budgeting content.
                  </p>
                </section>
              </aside>
            </section>
