...
export async function checkUserCoupleBudgetingClaim(): Promise<boolean> {
  try {
    ...
    console.error(
      "❌ Unexpected error checking couple budgeting claim status:",
      error,
    );
    return false;
  }
 ...
 */
export async function joinCoupleBudgetingWaitlist(
  claim: CoupleBudgetingClaim,
): Promise<CoupleBudgetingResponse> {
  try {
