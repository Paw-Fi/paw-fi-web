...
export function UserGeoMap({ data }: UserGeoMapProps) {
  ...
  const mappedUsers = useMemo(() => {
    return markers.reduce((sum, m) => sum + m.userCount, 0);
  }, [markers]);

  const coveragePercent = totalUsers > 0 ? Math.round((mappedUsers / totalUsers) * 100) : 0;
    ...
    <div className="relative w-full">
      ...
      <div className="pointer-events-none absolute inset-0">
        <svg className="h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet">
          ...
          {markers.map((marker, index) => {
            const size = getMarkerSize(marker.userCount);
            const pulseClass = marker.userCount > 100 ? "animate-pulse" : "";
              ...
              <g key={marker.timezone} className={pulseClass}>
                ...
                <g transform={`translate(${marker.x + size / 20}, ${marker.y - size / 20})`}>
                  <rect
                    x="0"
                    y="0"
                    width={Math.max(12, String(marker.userCount).length * 3.5) / 10}
                    height="4"
                    rx="2"
                  ...
                  />
                  <text
                    x={Math.max(12, String(marker.userCount).length * 3.5) / 20}
                    y="2.8"
                    textAnchor="middle"
                    fontSize="2.5"
                    fill="#34D399"
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-4 text-sm">
        <div className="text-white/60">
          <span className="font-medium text-white">{markers.length}</span> timezones with users
        </div>
        <div className="text-white/60">
          Coverage: <span className="font-medium text-emerald-400">{coveragePercent}%</span>
          <span className="ml-2 text-white/40">
            ({mappedUsers.toLocaleString()} / {totalUsers.toLocaleString()} users mapped)
          </span>
        </div>
      </div>
    </div>
