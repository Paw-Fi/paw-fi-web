...
export function ArcTimeline(props: ArcTimelineProps) {
  ...
  return (
    <>
      {/* Fix for global mobile-ux.css transition override */}
      <style>{`
        .arc-timeline-container .absolute {
          transition-property: transform, background-color, border-color, color, fill, stroke, opacity !important;
          transition-duration: 500ms !important;
          transition-timing-function: ease-in-out !important;
        }
        .arc-timeline-container div[style*="transform"] {
          transition: transform 500ms ease-in-out !important;
        }      
      `}</style>
      <div
        {...restProps}
        className={cn("relative h-[380px] w-full overflow-hidden arc-timeline-container", className)}
      >
        <div
          style={{
